...
export default function MoreProject() {
  // Data array remains the same for easy management
  const moreProjects = [
    {
  ...
  return (
    ...
    <section className="relative w-full overflow-hidden bg-white py-24 sm:py-32">
      {/* Subtle top separator line for white background */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent"></div>

      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        {/* Section Header - Typography changed to dark for contrast */}
        ...
        <div className="flex flex-col gap-16">
          {moreProjects.map((project, index) => (
            ...
            >
              {/* Content Area - Text remains light to contrast with dark card */}
              <div className="flex flex-1 flex-col justify-center p-8 sm:p-12 z-10">
              ...
              {/* Image/Mockup Area - Remains dark theme */}
              <div className="relative min-h-[300px] flex-1 overflow-hidden p-6 sm:p-8 lg:min-h-full bg-white/[0.02]">
                <div className="absolute inset-0 bg-gradient-to-br from-violet-900/10 via-transparent to-indigo-900/10 opacity-50"></div>

                {/* Browser Mockup window */}
                <div className="relative h-full w-full rounded-xl border border-white/10 bg-black shadow-2xl flex flex-col overflow-hidden transition-transform duration-700 group-hover:scale-[1.03]">
                  {/* Browser Top Bar with interactive dots */}
                  <div className="flex items-center gap-1.5 border-b border-white/10 bg-white/5 px-4 py-3">
                    <div className="h-2.5 w-2.5 rounded-full bg-slate-600 group-hover:bg-[#FF5F56] transition-colors duration-300"></div>
                    <div className="h-2.5 w-2.5 rounded-full bg-slate-600 group-hover:bg-[#FFBD2E] transition-colors duration-300 delay-75"></div>
                    <div className="h-2.5 w-2.5 rounded-full bg-slate-600 group-hover:bg-[#27C93F] transition-colors duration-300 delay-150"></div>
                  </div>

                  {/* Mockup Image Area */}
                  <div className="flex-1 bg-[url('https://res.cloudinary.com/dvvcwzp4n/image/upload/f_webp/14375_c4sbqw')] bg-cover bg-center opacity-40 group-hover:opacity-60 transition-opacity duration-500"></div>
                </div>
              </div>
            </div>
          ))}
