'use client';
// React Bits SplashCursor, manually installed from the user-supplied TS source.
// Adapted to a section-local canvas, pointer events, and managed WebGL lifecycle.
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    if (!canvas || !scope || reducedMotion) return;
    ...
    function track<T>(resource: T | null, collection: Set<T>): T | null {
      if (resource) collection.add(resource);
      return resource;
    }
    ...
    const context = (() => {
      try { return getWebGLContext(canvas); } catch { return null; }
    })();
    if (!context) return;
    ...
    // This WebGL class intentionally closes over this effect's context.
    ...
    const blit = (() => {
      const buffer = track(gl.createBuffer(), buffers)!;
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
      gl.bufferData(gl.ARRAY_BUFFER, new Float32Array([-1, -1, -1, 1, 1, 1, 1, -1]), gl.STATIC_DRAW);
      const elemBuffer = track(gl.createBuffer(), buffers)!;
    ...
    function updateFrame() {
      ...
      step(dt);
      render(null);
      if (performance.now() - lastInputTime < 4000) frame = requestAnimationFrame(updateFrame);
    }
    ...
    function coordinates(event: PointerEvent) {
      ...
      return [scaleByPixelRatio(event.clientX - rect.left), scaleByPixelRatio(event.clientY - rect.top)];
    }
    let inside = false;
    function move(event: PointerEvent) {
      ...
      if (!inside) {
        ...
        inside = true;
      } else updatePointerMoveData(pointer, x, y, pointer.color);
      lastInputTime = performance.now();
      wake();
    }
    function down(event: PointerEvent) {
      const [x, y] = coordinates(event);
      updatePointerDownData(pointers[0], event.pointerId, x, y);
      inside = true;
      clickSplat(pointers[0]);
      lastInputTime = performance.now();
      wake();
    }
    function leave() { inside = false; updatePointerUpData(pointers[0]); }
    function pause() { cancelAnimationFrame(frame); frame = 0; inside = false; }
    const observer = new IntersectionObserver(([entry]) => {
      visible = entry.isIntersecting;
      if (visible) wake(); else pause();
    });
    observer.observe(scope);
    const resizeObserver = new ResizeObserver(() => { lastInputTime = performance.now(); wake(); });
    resizeObserver.observe(scope);
    function visibilityChange() { if (document.hidden) pause(); else wake(); }
    ...
    scope.addEventListener('pointerup', leave);
    ...
    return () => {
      ...
      scope.removeEventListener('pointerup', leave);
  ...
  return (
    <div aria-hidden="true" data-hero-splash className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
